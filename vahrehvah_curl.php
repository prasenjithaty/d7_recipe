...
$main_content = getContent($url);
$node_results = array();


$recipe_types = "";
$recipe_types = extractContent('%<li><a href="indianrecipes.php" class="dir".*?</ul>.*?</li>%sim', $main_content, 0);
$recipe_types = preg_replace('/<li><a href="indianrecipes.php".*?<ul>/sim', '', $recipe_types);
$recipe_types = preg_replace('%</ul>.*%sim', '', $recipe_types);

$recipe_cat_urls = extractContentAll('/href=".+?"/sim', $recipe_types, 0);

$recipe_cat_titles = extractContentAll('%">.+?</a>%sim', $recipe_types, 0);
...
foreach ($recipe_cat_urls as $key => $recipe_cat_url) {
    ...
    $recipe_cat_content = getContent($recipe_cat_url_final);
    $recipe_cat_content = extractContent('%<table align="center" width="730" border="0" >.*?</table>%sim', $recipe_cat_content, 0);

    $single_recipe_title_url = extractContentAll('%<a href=".*?</a>%sim', $recipe_cat_content, 0);
    $inner_index = 0;
...
function  getVahRehVahRecipeNode($single_recipe_url, $category) {
    $single_recipe_url = extractContentAll('/\b(?:(?:http?):\/\/|www\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/sim', $single_recipe_url, 0);
    $single_recipe_url = $single_recipe_url[0];

    $recipe_content = getContent($single_recipe_url);
    $recipe_details = extractContentAll('%<fieldset id="Category - Parent Category_fieldset" class="admin_fieldset" style="text-align:center;width:560px;" record_number="34">.*?</fieldset>%sim', $recipe_content, 0);
    $recipe_metadata = $recipe_details[0];
    $recipe_details = $recipe_details[1];

    $recipe_title = extractContent('%<h1[^>]*>(.*?)</h1>%sim', $recipe_metadata, 1);
        
    $recipe_details_filtered = preg_replace('%<tr class="ingredient"[^>]*>(.*?)</tr>%sim', '', $recipe_details);
    $ingredients = extractContentAll('%<td align="left"  style="width:[^>]*>(.*?)<br />%sim', $recipe_details_filtered, 1);
    $ingredients = array_chunk($ingredients, 3, TRUE);

    $recipe_instructions = extractContent('%<td[^>]*class="instructions">(.*?)</td>%sim', $recipe_details_filtered, 1);
    $recipe_instructions = preg_replace('%<br />%sim', '', $recipe_instructions);

    $recipe_metadata_filtered = extractContent('%<table cellpadding="2"[^>]*>(.*?)</table>%sim', $recipe_metadata, 0);
    ...
    $recipe_summary = extractContent('%<span itemprop="summary"[^>]*>(.*?)</span>%sim', $recipe_metadata, 1);
...
function extractContentAll($reg_exp, $content, $index) {
